const connect_to_db = require('./db_TalksByAuthor'); //carico il DB, carica il file db.js

// GET BY TALK HANDLER

const talk = require('./Talks_TalksByAuthor'); //carico il modello dati nel file Talk.js

//gestisco l'evento
//evento
//contesto: non lo useremo
//callback: 
module.exports.get_by_author = (event, context, callback) => {
    ...
    if(!body.main_speaker) {
        ...
        callback(null, {
                    ...
                    body: 'Could not fetch the talks. main_speaker is null.'
        })
    }
    ...
    connect_to_db().then(() => {
        ...
        talk.find({main_speaker: {'$regex': body.main_speaker, '$options': 'i'}}) // cerco autori contententi il nome parziale in input, CASE UNSENSITIVE
